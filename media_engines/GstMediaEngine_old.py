...
demux.
    ...
    def start(self):
        ...
        if self.decode_video:
            ...
            desc = self.PIPELINE %(demux) + self.VIDEO_DEC %(parse, q, v_sink) #'''playbin'''
        else:
            desc = self.PIPELINE %(demux) + self.VIDEO_NODEC %(q, v_sink)
        #debug(DEBUG, '%s pipeline: %s', self, desc)
	desc = self.PIPELINESRC
        self.pipeline = gst.parse_launch(desc)
	#self.pipeline = gst.element_factory_make("playbin2","pipeline")
        #self.pipeline.set_property("uri", "appsrc://")
	#self.pipeline = gst.parse_launch ("playbin uri=appsrc://",None);
	#self.pipeline.app_source= gst.element_factory_make("appsrc", "vidsrc")
    ...
    def on_source_setup(self, element, source):
        ...
        source.connect("push-buffer", self.pushData)
    ...
    def pushData(self,data, fragment_duration, level, caps_data):
        ...
        debug(DEBUG, '%s pushData: pushed %s of data (duration= %ds) for level %s', self, 
            ...
            fragment_duration,
            level)
	self.pipeline.emit('push-buffer', buf);
	#self.pipeline.get_pad("src").emit('push-buffer', buf)
	#self.pipeline.get_by_name('src').emit('push-buffer', buf)        
	#self.emit('push-buffer', buf)
	#self.pipeline.get_by_name('source').emit('push-buffer', buf)
	#self.pipeline.source.emit('push-buffer', buf)
	#print self.pipeline.get_property('buffer-duration')
        del buf
	...
	return self.pipeline.get_property('buffer-size')
        #return self.queue.get_property('current-level-bytes')

    def getQueuedTime(self):
	return self.pipeline.get_property('buffer-duration')*1e-9
