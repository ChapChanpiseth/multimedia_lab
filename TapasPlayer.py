...
class TapasPlayer(object):
	...
	url=playlist['segments'][self.cur_index]['url']
	return url

    def get_byterange(self):
	playlist = self.parser.playlists[self.getCurrentLevel()]
	byterange=playlist['segments'][self.cur_index]['byterange']
	return byterange
    ...
    def fetchNextSegment(self): 
        ...
        if self.getCurrentSegmentIndex() > playlist['end_index']:
            self.setCurrentSegmentIndex(playlist['end_index'])
...
#                    self.setLevel(self.controller.getControlAction()) CHANGED HERE IUT
		    self.setLevel(self.getCurrentLevel()-1)
    ...
    def log(self):
        ...
        opts = dict(
           ...
           bwe=self.getBandwidth()*8/1000000,                         #4
           cur=self.getCurrentRate()*8/1000000,                       #5
	   ...
	   url_segment=self.get_url(),
	   byterange=self.get_byterange(),
	   fragment_duration=self.parser.getFragmentDuration(),
        )
        levels = self.parser.getLevels()
        for i in range(0,len(self.getLevelRates())):
           opts['q%d' %i] = float(levels[i]['rate'])*8/1000000
...
#CHANGED HERE IUT (not the nxt 5 lines normally):
	    if self.getCurrentLevel()-1>-1:
	        lev=self.getCurrentLevel()-1
	    else:
		lev=0
	    self.setLevel(lev)
            self.fetchNextSegment()
